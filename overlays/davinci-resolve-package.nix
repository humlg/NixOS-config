# Upstream nixpkgs davinci-resolve package.nix, modified to track v21.0b1.
#
# Changes vs upstream (nixos-unstable, was 20.3.2):
#   - version bumped to "21.0b1"
#   - free-variant outputHash updated (ZIP hash verified locally)
#   - studio-variant outputHash marked as unverified (not used in this config)
#   - src fetch script: downloadId is now hardcoded (bypasses the jq title-match
#     lookup that breaks because the 21 Beta 1 title is "DaVinci Resolve 21 Beta 1",
#     not "DaVinci Resolve 21.0b1")
#   - jq / DOWNLOADSURL / VERSION removed from src derivation accordingly
#
# To revert to the nixpkgs version, remove this file and its overlay entry.
{
  stdenv,
  lib,
  cacert,
          ...
          rec {
            ...
            outputHash =
              if studioVariant then
                # Studio hash NOT verified for v21.0b1 — download and update if needed.
                # Studio downloadId: 03fbd8ecbcec4bb1a1c9b36a543b35be
                lib.fakeHash
              else
                "sha256-qQ6jYUYX6qd4foGLYqg+eTBrHvQteYJddRvx9y8CGps=";
            ...
            # Hardcoded downloadIds for v21.0b1 (from downloads.json, 2026-04-27).
            # The jq title-match in the upstream src script looks for
            # "DaVinci Resolve <version>" but the 21 Beta 1 entry is titled
            # "DaVinci Resolve 21 Beta 1", so we skip that lookup and use the
            # known downloadId directly.
            DOWNLOADID =
              if studioVariant
              then "03fbd8ecbcec4bb1a1c9b36a543b35be" # Studio 21 Beta 1 (hash unverified)
              else "fc26aa1232f34a0daa60d257cdd5b962"; # Free 21 Beta 1
