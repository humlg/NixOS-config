...
modules/
  ...
  services/
    ...
    wireguard-saruman.nix          # Personal WireGuard VPN tunnel (networking.wg-quick.interfaces.wg-homelab), configFile sourced from the whole wg-homelab.age secret rather than split into individual options — full-tunnel (0.0.0.0/0), so autostart = false; bring up manually with `sudo wg-quick up wg-homelab`. Interface named wg-homelab, not wg-saruman, so the connection name doesn't just repeat the hostname. custom.wireguard-saruman.enable — saruman only
  system/
