...
| saruman | AMD Ryzen AI 9 365 (Strix Point, 10C/20T), Lenovo IdeaPad 14ASP9 / Yoga Pro 7 14ASP9 | AMD Radeon 880M iGPU (RDNA 3.5, DCN 3.5) | MediaTek MT7922 (`mt7921e`) | RDNA 3.5 not yet officially supported by ROCm — `HSA_OVERRIDE_GFX_VERSION=11.0.0`. **s2idle sleep/resume hang — mitigated 2026-08-16 by a local kernel patch, recurred 2026-08-17 (rate reduced, not eliminated).** Upstream kernel bug [#219445](https://bugzilla.kernel.org/show_bug.cgi?id=219445) (exact same laptop model) bisected it to `f6098641d3e1e4` ("drm/amd/display: fix s2idle entry for DCN3.5+"), which makes `dm_suspend()` force the display block into IPS before D3cold. The bisect was right; the original mitigation was aimed at the wrong field. `amdgpu.dcdebugmask=0x800` (`DC_DISABLE_IPS`) sets `dc->config.disable_ips`, while the offending call is guarded by `dc->caps.ips_support` — a hardware capability bit — so the parameter never touched that code path, which is why it **failed to fix the hang on 2026-07-22**. The fix is `patches/amdgpu-no-idle-opt-on-s2idle.patch` (applied via `custom.amdgpu-s2idle-patch.enable`, `modules/system/amdgpu-s2idle-patch.nix`), deleting those two lines — the same removal independently confirmed to fix identical hangs on Ryzen AI 7 350 / AI 5 340 (same DCN3.5 block) in the amd-gfx "Radeon 840M/860M bisected suspend crash" thread. Trade-off, accepted: kernel builds locally on every version bump (~15–25 min), and the part no longer reaches its deepest hardware sleep, so suspended drain is higher. Hang signature for future reference: journal's last line is `PM: suspend entry (s2idle)` with no `PM: suspend exit`; it needs >10 min of sleep residency, so short `rtcwake` cycles never reproduce it. **Looked validated 2026-08-16** — 7 s, 22 min and 60 min s2idle cycles all resumed cleanly — **but the hang recurred 2026-08-17**: `PM: suspend entry (s2idle)` as the last journal line again, no exit, machine dead ~15h until the next hard-boot. Same boot, two earlier cycles that day had resumed fine, so this looks like a reduced hang rate (previously ~22%), not zero — three clean cycles wasn't enough to prove elimination. No further change made yet; gathering more `journalctl -b N -k | grep -c "PM: suspend entry"` vs `"...exit"` data across boots before deciding on next steps (see `maintenance.md` item 7). **A second, separate bug rules hibernation out on this machine:** hibernate *resume* restores its image fine and then dies in TTM — `list_add corruption` → `ttm_bo_populate` → `ttm_resource_add_bulk_move`, wedging the GPU (hyprlock draws one frame, then no VT switch, hard power-off needed). That path is only reachable via `ttm_device_prepare_hibernation()`, which amdgpu calls only under `adev->in_s4`, so avoiding hibernation avoids it. **All four sleep paths are therefore plain s2idle `suspend`** as of 2026-08-17 — lid close, power key (`HandlePowerKey`), hypridle's 30-min idle timeout (`desktop.hyprland-desktop.sleepCommand`), and undock-with-lid-shut (`custom.lid-undock-hibernate.sleepCommand`). Nothing hibernates automatically; `boot.resumeDevice` and the swap stay for a manual `systemctl hibernate` and so the backstop can be restored later. Accepted trade: no protection against a flat battery on a long unattended absence, on a kernel that already draws more in standby than stock — measure drain before trusting it to a weekend. Both `sleepCommand` options still default to suspend/hibernate respectively so sauron is unaffected (it has swap but no `boot.resumeDevice`). Lid-switch handling must be set via `services.logind`, never a Hyprland compositor keybind — logind's own native lid handler fires independently and races any compositor-level bind. Lid close **with an external monitor attached** is `HandleLidSwitchDocked = "ignore"` so the laptop stays usable lid-shut on the Iiyama — paired with `custom.lid-undock-hibernate.enable`, since logind evaluates the lid action only at the instant the lid event fires and never re-checks on undock. `mt7921e disable_aspm=1` kept as a harmless secondary mitigation (see `maintenance.md` item 7). `ucsi_acpi` blacklist tried and removed — confirmed not the cause of the sleep hang, and it disabled USB-C PD negotiation entirely, so it's loaded again. LUKS-encrypted swap (29.9GB, exceeds 27GiB RAM); hibernate images use LZ4 (`CONFIG_HIBERNATION_COMP_LZ4`, enabled by the same kernel-patch module) — untrusted, one of the two open suspects for the resume crash above; only the default changed, `CONFIG_CRYPTO_LZO=y` survives so `hibernate.compressor=lzo` is selectable at runtime without a rebuild. TPM 2.0 present but unused — LUKS unlock is passphrase-only by choice. BIOS PSCN23WW. |
