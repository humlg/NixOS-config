...
modules/
  ...
  desktop/
    ...
    hypr-dynamic-cursors.nix       # "Shake to find" — cursor grows when you wiggle the mouse (KDE Shake Cursor style). HM module, desktop.hyprland-desktop.dynamicCursors.enable. Loads pkgs.hyprlandPlugins.hypr-dynamic-cursors (nixpkgs, ABI-matched to pkgs.hyprland — never hyprpm) via wayland.windowManager.hyprland.plugins, runs it shake-only (mode = "none"), appends the plugin's hl.config block after the main Lua config with mkAfter + an hl.plugin.dynamic_cursors guard. Requires useLuaConfig = true (asserted). Enabled on sauron + saruman, off on nixosvm. See maintenance.md #22
