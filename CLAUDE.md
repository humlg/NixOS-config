...
modules/
  ...
  services/
    ...
    lid-undock-hibernate.nix       # Sleeps when the last external display is unplugged while the lid is already shut — closes the gap left by HandleLidSwitchDocked = "ignore" (logind only evaluates the lid action at the instant the lid event fires, never re-checks on undock) — opt-in via custom.lid-undock-hibernate.enable; the action is the sleepCommand option (defaults to hibernate, set to suspend-then-hibernate on saruman) — saruman only
